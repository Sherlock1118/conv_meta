...
class one_transfer(nn.Module):
    ...
    def __init__(self,input_dim,out_dim, kernel=2):
        ...
        self.fc1 = nn.Linear(input_dim*self.out_channel2,input_dim*out_dim*2) # 128
    ...
    def forward(self,x):
        ...
        x = Gelu(x)
        x = self.fc2(x)
        return x.view(input_dim, out_dim)
...
class atten_source_emb_fc(nn.Module):
    ...
    def forward(self, s_item, t_item):
        print('t_item.reshape((self.args.bridge_batch_size, 1, -1)).transpose(0, 1)', t_item.reshape((self.args.bridge_batch_size, 1, -1)).transpose(0, 1).shape)
        print('s_item.reshape((self.args.bridge_batch_size, 1, -1)).transpose(0, 1)', s_item.reshape((self.args.bridge_batch_size, 1, -1)).transpose(0, 1).shape)
...
class ConvTransfer_com(nn.Module):
    ...
    def forward(self, x_t, x_hat, s_item, t_item):
        ...
        x = torch.cat((s_item, x_t.view(-1), x_hat.view(-1), x_com),dim=0)
    ...
    def run_bridge(self, args, bridge_model, last_bridge_weight, bridge_weight_hat, 
        ...
        bridge_model.module.layer.weight.data.copy_(bridge_weight_new)
