...
#         self.bridge_base = bridgeModel(input_dim_bridge, out_dim_bridge).cuda()
        ...
        for name, p in self.bridge_base.module.named_parameters():
...
#         print('Model {} : params: {:4f}M'.format(self.bridge_base._get_name(), para * type_size / 1000 / 1000))
        ...
        self.bridge_weight = self.bridge_base.module.layer.weight.data
        if self.with_MF_bias: # input the bridge bais into transfer!
            self.last_bridge_weight = self.bridge_base.module.layer.weight.data
            self.bridge_weight_hat = self.bridge_base.module.layer.weight.data
            self.last_bridge_weight_hat = self.bridge_base.module.layer.weight.data
            
            input_dim_bridge = laten_dim + 1
        else:                # don't input the bridge bais into transfer!
            input_dim_bridge = laten_dim

            self.last_bridge_weight = torch.zeros_like(self.bridge_base.module.layer.weight.data)
            self.item_embd = torch.zeros_like(torch.empty(args.bridge_batch_size, 1, input_dim_bridge))
            self.bridge_weight_hat = copy.deepcopy(self.bridge_base.module.layer.weight.data)
            self.last_bridge_weight_hat = copy.deepcopy(self.bridge_weight_hat)
            

        ''' creat meta transfer model'''
        self.transfer = nn.DataParallel(ConvTransfer_com(args, input_dim_bridge, input_dim_bridge,  args.s_item_dimention, args.t_usr_dimention, args.t_item_dimention), device_ids=[0,1]).cuda()
        for name, p in self.transfer.module.named_parameters():
            p_size = np.prod(list(p.size()))
            print('Model {} : params: {:4f}M'.format(name, p_size * type_size / 1000 / 1000))
...
#         self.dataset = datasets
        ...
        self.bridge_optimizer = torch.optim.Adam(self.bridge_base.module.parameters(), lr=args.bridge_lr, weight_decay=0)
        self.transfer_optimizer = torch.optim.Adam(self.transfer.module.parameters(), lr=args.TR_lr, weight_decay=args.TR_l2) #1
...
#                 labels = labels.long()
                ...
                bridge_weight_hat = self.bridge_base.module.layer.weight.data


                loss_batch = self.transfer.module.run_bridge(args, copy.deepcopy(self.bridge_base), last_weight_bridge,
                                                  bridge_weight_hat,
...
#                 labels = labels.long()
    ...
    def save_bridge_weight(self, save_as="last"):
        ...
        if self.with_MF_bias:
            ...
            if save_as == "last":
                self.last_bridge_weight.copy_(self.bridge_base.module.layer.weight.data)

            elif save_as == "hat":
                self.last_bridge_weight_hat.copy_(self.bridge_weight_hat.data)
                self.bridge_weight_hat.copy_(self.bridge_base.module.layer.weight.data)
            else:
    ...
    def load_bridge_weight(self, bridge_weight):
        ...
        :return: None
        '''
        self.bridge_base.module.layer.weight.data.copy_(bridge_weight)
