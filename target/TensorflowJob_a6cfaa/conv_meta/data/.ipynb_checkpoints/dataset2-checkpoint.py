...
#         self.dataset_conv_meta = dataset_meta_conv
    ...
    def get_each_dataset(self, mode=None):
        if mode == 'support':
            ...
            train_dataset_builder = DatasetBuilder(input_table=self.args.input_table_train,\
            ...
            del train_dataset_builder, train_dataset_builder_, s_usr_embed_df, s_item_embed_df, t_usr_cat_df, t_usr_num_df, t_item_cat_df, t_item_num_df, label_df, t_usr_cat_df_prep, t_usr_df, t_item_df
            return dataset_conv_meta
        elif mode == 'query':
            spec = self.featuers_spec()
            test_dataset_builder = DatasetBuilder(input_table=args.input_table_test, \
            partitions=f'pt={self.args.pt}', column_spec=spec).to_pandas()
            ...
            t_item_df = pd.concat([t_item_cat_df_prep, t_item_num_df], axis=1)
            dataset_conv_meta = dataset_meta_conv(s_usr_embed_df, s_item_embed_df, t_usr_df, t_item_df, label_df)
            del test_dataset_builder,test_dataset_builder_, s_usr_embed_df, s_item_embed_df, t_usr_cat_df, t_usr_num_df, t_item_cat_df, t_item_num_df, label_df, t_usr_cat_df_prep, t_usr_df, t_item_df
            return dataset_conv_meta
        else:
            return None
    ...
    def get_dataLoader(self):
        ...
        del self.train_dataset_builder, self.test_dataset_builder
