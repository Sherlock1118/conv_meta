...
class meta_train():
    ...
    def __init__(self,args,input_dim_bridge, out_dim_bridge, laten_dim):
        ...
        type_size=4
        # bridge model
        self.bridge_base = bridgeModel(input_dim_bridge, out_dim_bridge).cuda()
#         self.bridge_base = nn.DataParallel(bridgeModel(input_dim_bridge, out_dim_bridge), device_ids=[0,1]).cuda()
        for name, p in self.bridge_base.named_parameters():
            p_size = np.prod(list(p.size()))
            print('Model {} : params: {:4f}M'.format(name, p_size * type_size / 1000 / 1000))
#         para = sum([np.prod(list(p.size())) for p in self.bridge_base.parameters()])
#         print('Model {} : params: {:4f}M'.format(self.bridge_base._get_name(), para * type_size / 1000 / 1000))
        ...
        self.s_item_embd = torch.zeros_like(torch.empty(args.bridge_batch_size, 1, args.s_item_dimention))
        self.t_item_embd = torch.zeros_like(torch.empty(args.bridge_batch_size, 1, args.t_item_dimention))
        ...
        ''' create meta transfer model'''
        self.transfer = ConvTransfer_com(args, input_dim_bridge, out_dim_bridge,  args.s_item_dimention, args.t_usr_dimention, args.t_item_dimention).cuda()
#         self.transfer = nn.DataParallel(ConvTransfer_com(args, input_dim_bridge, out_dim_bridge,  args.s_item_dimention, args.t_usr_dimention, args.t_item_dimention), device_ids=[0,1]).cuda()
        for name, p in self.transfer.named_parameters():
            p_size = np.prod(list(p.size()))
            print('Model {} : params: {:4f}M'.format(name, p_size * type_size / 1000 / 1000))
#         para = sum([np.prod(list(p.size())) for p in self.transfer.parameters()])
#         print('Model {} : params: {:4f}M'.format(self.transfer._get_name(), para * type_size / 1000 / 1000))
#         self.transfer = ConvTransfer_com(args, input_dim_bridge, input_dim_bridge,  args.s_item_dimention, args.t_usr_dimention, args.t_item_dimention).cuda()
#         self.dataset = datasets
    ...
    def bridge_train_onestage(self,args,surpport_set,stage_id,val=None): # this right for main
        ...
        for epoch in range(args.bridge_epochs):
            ...
            loss_all = 0
            for batch_id,(source_user_embd, source_item_embd, t_user, t_item, labels) in enumerate(surpport_set):
...
#                 labels = labels.long()
                ...
                self.s_item_embd = source_item_embd_cuda
                self.t_item_embd = target_item_cuda
                last_weight_bridge = self.last_bridge_weight
                bridge_weight_hat = self.bridge_base.layer.weight.data


                loss_batch = self.transfer.run_bridge(args, self.bridge_base, last_weight_bridge,
                                                  bridge_weight_hat,
                ...
                del source_user_embd, source_item_embd, t_user, t_item, labels

            loss_all = loss_all/(batch_id + 1)
            loss_all = loss_all.item() / args.bridge_batch_size
            self.bridge_itr+=1
            ...
            if val is not None:
                ...
                if self.need_writer:
                    self.writer.add_scalar("Loss/bridge-loss",loss_all, self.bridge_itr)
...
#                 labels = labels.long()
                ...
                loss_batch, labels_, labels_pred = self.transfer.run_bridge(args, self.bridge_base, last_weight_bridge,
                                                  bridge_weight_hat,
                                                  source_user_embd_cuda,
                                                  source_item_embd_cuda,
                                                  target_user_cuda,
                                                  ...
                                                  labels,
                                                  mode='transfer')
                labels_this += labels_.view(-1).tolist()
                label_pred += labels_pred.view(-1).tolist()
                loss_all += loss_batch.data
                loss_batch.backward()
                # if clip:
            ...
            auc = roc_auc_score(labels_this, label_pred)
            acc = accuracy_score(labels_this, label_pred)
            print('AUC: ', auc)
            print('Accuracy: ', acc)
            print(classification_report(labels_this, label_pred))
    ...
    def train_one_stage3(self,args,stage_id):
        ...
        for phase in range(args.multi_num): # outer loop
            ...
            # 更新Wt
            self.updata(self.s_item_embd, self.t_item_embd)
    ...
    def updata(self, s_item_embd, t_item_embd):
        '''
        ...
        self.bridge_base.eval()
        self.transfer.eval()
        new_bridge_weight = self.transfer(self.last_bridge_weight,self.bridge_weight_hat, s_item_embd, t_item_embd)
